...
const { trigger: updateUser, isMutating: isUpdating } = useSWRMutation<
  string,                  // Data (we're not using it here, so can be string or null)
  any,                     // Error
  string,                  // Key
  Partial<User>            // Extra Argument (the type of arg you will pass in)
>(
  `/api/admin/users/${userId}`,
  async (url, { arg }) => {
    const res = await fetch(url, {
  ...
  useEffect(() => {
    ...
    setValue('name', user.name);
    setValue('email', user.email);
    setValue('role', user.role); // <- set role here instead of isAdmin
  }, [user, setValue]);
  ...
  return (
    <div>
      ...
      <form onSubmit={handleSubmit(formSubmit)}>
        ...
        <div className='my-3 md:flex'>
          <label className='label md:w-1/5' htmlFor='role'>
            Role
          </label>
          <div className='md:w-4/5'>
            <select
