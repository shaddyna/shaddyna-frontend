...
const MembersPage = () => {
  ...
  const handleDelete = async (id: string) => {
    try {
      await axios.delete(`https://shaddyna-backend.onrender.com/api/startups/${id}`);
  ...
  const handleEdit = (id: string) => {
    router.push(`/edit-startup/${id}`);
  };

  return (
    <div>
        <Back title={"Startups List"} />
        ...
        {members.length === 0 ? (
          ...
          members.map((member) => (
            <div key={member._id} className="bg-white shadow-lg rounded-lg overflow-hidden border border-gray-200">
              <div className="p-6">
                <p className="text-xl font-semibold mb-2 text-[#182155]">{member.startupName}</p>
                <p className="text-gray-600 mb-2">{member.ideaDescription}</p>
                <p className="text-gray-600 mb-2">{member.estimatedValue}</p>
                <p className="text-gray-600 mb-2">{member.contactEmail}</p>
                <p className="text-gray-600 mb-2">{member.contactPhone}</p>
