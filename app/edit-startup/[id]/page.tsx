...
const EditStartupPage = () => {
      ...
      const fetchMember = async () => {
        try {
          const response = await axios.get(
            `https://shaddyna-backend.onrender.com/api/startups/${extractedId}`
          );
  ...
  const handleSubmit = async (e: { preventDefault: () => void }) => {
    ...
    try {
      const updatedMember = await axios.put(
        `https://shaddyna-backend.onrender.com/api/startups/${id}`,
        member
      );
      window.location.href = "/startup-list"; // Redirect to the members list after successful update
    } catch (error) {
  ...
  return (
    <div>
        <Back title={"Edit Startup"} />
    <div className="bg-gray-50">
    <div className="max-w-4xl mx-auto p-3">
      <div className="bg-white shadow-xl rounded-lg overflow-hidden border border-gray-200">
        <div className="p-6">
        <h1 className="text-3xl font-semibold text-[#182155] mb-3 text-center">Startup</h1>
          <form onSubmit={handleSubmit} className="space-y-3">
            ...
            <div className="flex flex-col sm:flex-row sm:space-x-6 sm:space-y-0">
              <div className="w-full sm:w-1/2">
                <label className="block text-[#182155] font-medium mb-2">Description</label>
                <input
                  type="text"
                  name="description"
                  value={member.ideaDescription}
                  onChange={handleChange}
                  className="w-full text-gray-700 px-4 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-[#182155] focus:border-[#182155]"
                />
              </div>
              <div className="w-full sm:w-1/2">
                <label className="block text-[#182155] font-medium mb-2">Value</label>
                <input
                  type="number"
                  name="value"
            ...
            <div className="flex flex-col sm:flex-row sm:space-x-6 sm:space-y-0">
              ...
              <div className="w-full sm:w-1/2">
                ...
                >
                  ...
                  <option value="non">Nonstartup</option>
                </select>
              </div>
            </div>
