...
const ShelvesPage = () => {
  ...
  const handleInvestment = (shelfId: string, amount: string) => {
    router.push(`/investment/${shelfId}?amount=${amount}`);
  };
  ...
  return (
    <div>
      <Back title="Shelves List" />
      <div className="container bg-white mx-auto p-3">
        {loading && <p className="text-center text-black">Loading shelves...</p>}
        {error && <p className="text-center text-red-500">{error}</p>}
        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
          {shelves.map((shelf) => (
            ...
            >
              ...
              <h2 className="text-xl text-blue-900 font-semibold mt-3">{shelf.name}</h2>
              <p className="text-gray-900">{shelf.description}</p>
              <p className="text-lg font-bold text-blue-900 mt-2">Ksh{shelf.price}</p>
