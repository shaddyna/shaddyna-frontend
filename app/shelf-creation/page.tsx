...
const AddShelfPage = () => {
  ...
  const onSubmit = async (data: Shelf) => {
    ...
    if (imageFile) {
      console.log("Appending image file:", imageFile);  // Log image to ensure it's being appended correctly
          ...
          <div>
            ...
            {fields.map((member, index) => (
              <div key={member.id} className="flex space-x-2 mt-2">
                <input {...register(`members.${index}.name`, { required: true })} type="text" placeholder="Member Name" className="w-full p-2 border rounded-md" />
                <input {...register(`members.${index}.role`, { required: true })} type="text" placeholder="Role" className="w-full p-2 border rounded-md" />
                {index > 0 && <button type="button" onClick={() => remove(index)} className="text-red-500 font-bold">X</button>}
              </div>
            ))}
            <button type="button" onClick={() => append({ id: fields.length + 1, name: "", role: "" })} className="mt-2 text-blue-600 font-semibold">+ Add Member</button>
          </div>

          <button type="submit" className="w-full bg-blue-600 text-white font-bold py-2 rounded-md hover:bg-blue-700" disabled={isSubmitting}>
