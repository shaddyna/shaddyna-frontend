...
import { Star, MapPin, ShoppingBag, ChevronDown, ChevronUp, Mail, Phone, Instagram, Facebook, Twitter, Search, X } from "lucide-react";
import NavbarTwo from "@/components/HeaderTwo";
import LuxuryFooter from "@/components/LuxuryFooter";
...
const fetchProducts = async () => {
  try {
    const response = await fetch(`http://localhost:5000/api/products?shopId=${shopId}`);
    if (!response.ok) {
      throw new Error('Failed to fetch products');
    }
    const data = await response.json();

    console.log('Fetched products:', data.products); // Log only the array

    setProducts(data.products); // ✅ Only store the array
  } catch (err) {
    ...
    <div className="bg-white min-h-screen">
      ...
      <div className="relative h-96 w-full overflow-hidden">
        ...
        <div className="absolute bottom-0 left-0 right-0 p-8 max-w-7xl mx-auto">
          ...
          <div className="flex items-center gap-2 mt-2">
            <MapPin className="text-[#bf2c7e]" size={20} />
            <span className="text-white">{shop.location}</span>
          </div>
        </div>
      </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <div className="lg:col-span-2">
            ...
            {(shop.policies.returnPolicy || shop.policies.shippingPolicy) && (
              <div className="bg-white rounded-xl shadow-md p-6 mb-8">
                <h2 className="text-2xl font-bold mb-4">Shop Policies</h2>
                
                {shop.policies.returnPolicy && (
                  <div className="mb-6">
                    <h3 className="font-semibold text-gray-900 mb-2">Return Policy</h3>
                    <p className="text-gray-700">{shop.policies.returnPolicy}</p>
                  </div>
                )}
                ...
                {shop.policies.shippingPolicy && (
                  <div>
                    ...
                    <p className="text-gray-700">{shop.policies.shippingPolicy}</p>
                  </div>
                )}
              </div>
            )}
          ...
          <div className="lg:col-span-1">
            <div className="bg-white rounded-xl shadow-md p-6 sticky top-8">
              <h2 className="text-xl font-bold mb-4">Contact & Hours</h2>
              
              <div className="space-y-4 mb-6">
                <div className="flex items-center gap-3">
                  <Mail className="text-[#bf2c7e]" size={18} />
                  <span>{shop.contact.email}</span>
                </div>
                
                {shop.contact.phone && (
                  <div className="flex items-center gap-3">
                    <Phone className="text-[#bf2c7e]" size={18} />
                ...
                <div className="mb-6">
                  ...
                  <div className="flex gap-4">
                    {shop.contact.instagram && (
                      <a href={shop.contact.instagram} target="_blank" rel="noopener noreferrer" className="text-gray-700 hover:text-[#bf2c7e]">
                        <Instagram size={20} />
                      </a>
                    )}
                    {shop.contact.facebook && (
                      <a href={shop.contact.facebook} target="_blank" rel="noopener noreferrer" className="text-gray-700 hover:text-[#bf2c7e]">
                        <Facebook size={20} />
                      </a>
                    )}
                    {shop.contact.twitter && (
                      <a href={shop.contact.twitter} target="_blank" rel="noopener noreferrer" className="text-gray-700 hover:text-[#bf2c7e]">
                        <Twitter size={20} />
                      </a>
                    )}
                  </div>
              ...
              <motion.button
                ...
                whileTap={{ scale: 0.98 }}
                className="w-full mt-6 bg-[#bf2c7e] hover:bg-[#0f1c47] text-black font-bold py-3 rounded-lg flex items-center justify-center gap-2"
              >
                <ShoppingBag size={18} />
                View Products
              </motion.button>
            ...
            <div className="sticky top-24 space-y-8">
              ...
              <div className="bg-white p-6 rounded-xl">
                ...
                <div className="space-y-4">
                  ...
                  <div className="flex items-start gap-4">
                    <div className="bg-[#bf2c7e]/10 p-2 rounded-lg">
                      <Clock className="text-[#bf2c7e]" size={20} />
                    </div>
                    <div>
                      <h3 className="font-medium">Established</h3>
                      <p className="text-gray-600">{new Date(shop.createdAt).toLocaleDateString()}</p>
                    </div>
                  </div>
              ...
              <div className="bg-white p-6 rounded-xl">
                ...
                <div className="space-y-4">
                  <div className="flex items-center gap-3">
                    <Mail className="text-gray-500" size={18} />
                    <a href={`mailto:${shop.contact.email}`} className="text-gray-700 hover:text-[#bf2c7e]">
                      {shop.contact.email}
                    </a>
                  </div>
                  ...
                  {shop.contact.phone && (
                    <div className="flex items-center gap-3">
                      <Phone className="text-gray-500" size={18} />
                      <a href={`tel:${shop.contact.phone}`} className="text-gray-700 hover:text-[#bf2c7e]">
                        {shop.contact.phone}
                      </a>
                    </div>
                  )}
                ...
                <div>
                  ...
                  {products.length === 0 && (
                    <div className="text-center py-12">
                      <p className="text-gray-600">No products available in this shop yet.</p>
                    </div>
                  )}
                </div>
              )}*/}
      ...
      {products.map((product, index) => (
        <ProductCard
          ...
          product={{
            ...
            designer: product.designer,
            price: product.price,
            originalPrice: undefined, // Adjust if you have original price data
            images: product.images,
            rating: 4.5, // Adjust if you have rating data
            isNew: false, // Adjust if you have isNew data
            stock: product.stock,
          }}
          theme={{
            primary: "#bf2c7e",
                ...
                <div className="space-y-8">
                  ...
                  <div>
                    ...
                    <div className="flex flex-col md:flex-row gap-6 items-start">
                      ...
                      <div className="flex-1">
                        <h4 className="font-bold text-lg">{shop.owner.name}</h4>
                        <p className="text-gray-600 mb-4">Shop Owner</p>
                        <p className="text-gray-700">
