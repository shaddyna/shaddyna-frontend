...
import { BubbleChatIcon, LocationUser03Icon, Notification03Icon, ShoppingCart01Icon, UserAccountIcon } from "hugeicons-react";
...
export const IconButtons = ({ isLoggedIn, totalItems, router }: IconButtonsProps) => (
  isLoggedIn ? (
    <div className="lg:flex items-center space-x-4">
      ...
      <button className="text-2xl p-2 text-[#182155] rounded-full">
        <UserAccountIcon />
      </button>
    </div>
  ) : null
...
/*"use client";
import React from "react";
import { BubbleChatIcon, Notification03Icon, ShoppingCart01Icon } from "hugeicons-react";
...
export const MobileIconButtons = ({ isLoggedIn, totalItems, router }: IconButtonsProps) => (
  isLoggedIn ? (
    ...
    <div className={`fixed bottom-0 left-0 right-0 bg-white rounded-t-2xl shadow-lg transform transition-all duration-500 ease-[cubic-bezier(0.32,0.72,0,1)] ${
      ...
      <div className="p-4">
        ...
        <div className="space-y-4">
          {[
            { icon: <UserAccountIcon className="mr-3" />, text: "My Profile", action: () => router.push("/profile") },
            { icon: <ShoppingCart01Icon className="mr-3" />, text: "Admin Panel", action: () => router.push("/orders") },
            { icon: <LocationUser03Icon className="mr-3" />, text: "Delivery", action: () => router.push("/addresses") },
            { icon: <UserAccountIcon className="mr-3" />, text: "Compare", action: () => router.push("/profile") },
            { icon: <ShoppingCart01Icon className="mr-3" />, text: "Chats", action: () => router.push("/orders") },
            { icon: <LocationUser03Icon className="mr-3" />, text: "Notifications", action: () => router.push("/addresses") },
            { icon: <UserAccountIcon className="mr-3" />, text: "My Shop", action: () => router.push("/profile") },
            { icon: <ShoppingCart01Icon className="mr-3" />, text: "Help", action: () => router.push("/orders") },
          ].map((item, index) => (
            <button
              key={index}
              onClick={item.action}
