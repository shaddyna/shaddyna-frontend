...
import { useEffect, useState } from "react";
import { useSearchParams } from "next/navigation";
import { fetchProducts, fetchShops, fetchUsers, Product, Shop, User } from "@/utils/api";
import { highlightMatch } from "@/utils/filterProducts";
...
export default function SearchResults() {
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      setLoading(true);
      try {
        const [fetchedProducts, fetchedShops, fetchedUsers] = await Promise.all([
          fetchProducts(),
          fetchShops(),
          fetchUsers(),
        ]);
        ...
        setShops(
          fetchedShops.filter(s => s.name?.toLowerCase().includes(query.toLowerCase()))
        );
        setUsers(
    ...
    <div className="container mx-auto p-4 bg-white">
      ...
      {products.length > 0 && (
        <Section title="Products">
          {products.map(product => (
            <Card key={product._id} imgSrc={product.images[0]} name={product.name} highlight={query} extra={`Ksh${product.price.toFixed(2)}`} />
          ))}
        </Section>
      )}
      ...
      {shops.length > 0 && (
        <Section title="Shops">
          {shops.map(shop => (
            <Card key={shop._id} imgSrc={shop.image} name={shop.name} highlight={query} />
          ))}
        </Section>
      )}
      ...
      {users.length > 0 && (
        <Section title="Users">
         ...
         key={user._id}
         imgSrc={user.avatar}
         name={`${user.firstName} ${user.lastName}`} // Use firstName and lastName
         highlight={query}
       />
